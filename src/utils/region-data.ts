...
export function getFullAddressName(code: string): string[] {
  const province = regionData.find(p => code.startsWith(p.value))
  if (!province) return []
  
  const city = province.children?.find(c => code.startsWith(c.value))
  if (!city) return [province.label]
  
  const district = city.children?.find(d => code === d.value)
...
export const getAreaName = (code: string): string => {
  ...
  // 查找区县
  const area = city.children?.find(a => a.value === areaCode)
